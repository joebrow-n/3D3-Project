...
void newAddress(struct pdu1 *pdu, int sd_tcp, int port, char* ip);
...
int main(int argc, char **argv)
{
	...
	newAddress(&rpdu, sd_tcp, SERVER_TCP_PORT, "127.0.0.1");                                                                  
...
void newAddress(struct pdu1 *pdu, int sd_tcp, int port, char* ip){
	...
	reg_addr.sin_port = htons(port); // Random Port #
	reg_addr.sin_addr.s_addr = inet_addr(ip); // IP Address of localhost
